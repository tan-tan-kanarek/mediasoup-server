...
class RtspServer extends EventEmitter {
	constructor(webRtcServer){
        ...
        .on('new-connection', (connection) => {
        	...
        	.on('new-stream', (stream) => {
        		let source = this.getSource(stream.peer.id, true);
        		source.addStream(stream);
        	})
        	.on('ready', (peerConnection) => {
        		let source = this.getSource(peerConnection.peer.id, true);
        		source.enable();
        	})
        	.on('close', (peerId) => {
	...
	getSource(sourceId, createIfNedded = false) {
		if(createIfNedded && !this.sources[sourceId]){
			this.addSource(new RtspSource(sourceId));
		}
